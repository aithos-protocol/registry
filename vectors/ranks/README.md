...
| Evidence resolution (§9.2) | Extract evidence automatically from the artifact; resolve an artifact view from its associated task; retrieve existing evidence through the configured A2A client; colliding artifact IDs in different tasks/providers are not confused; unavailable or conflicting evidence is refused; lookup never substitutes for a missing participant signature. |
| Rating (§5) | Both directions independently; correct rated role; self-rating and third-party rater rejected; accept 0, 0.1, 0.77, 0.123456 and 1; reject missing/string/non-finite values, negative scores, values above 1 and excess decimal precision; exact decimal-to-unit conversion; modified score, identities, result, artifact digest, log or timestamp invalidates original signature. |
...
| Read API and means (§8) | Stable bounded pagination during appends; absent rating distinct from score 0; both role means; failure scores included; decimal sums without binary accumulation error; six-place half-up rounding; integer-unit overflow; agent with no history; recompute filtered history and means from the full prefix; tampered summary not authenticated by its checkpoint alone. |
| Service/SDK (§9) | Body and decoded payload limits; safe errors; no remote key/content fetch; `rank(privateKey, artifact, score)` extracts context, verifies content and metadata, and uses only the supplied score; missing context/key/score rejected; signed failure bundle accepted without a synthetic artifact; private content and salts never submitted; return full confirmation package; reject mismatched acknowledgment; rating-service outage does not block business artifact delivery. |
